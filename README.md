...
* Watch a `Short YouTube Video` of Cupertino Icons Gallery App Here: [https://youtu.be/Aa59ucCQxhw](https://youtu.be/Aa59ucCQxhw)
...
### ⚙️ Generate the Latest Gallery Icons Data

Before running the app, make sure to regenerate `gallery_icons_data.dart` to include the latest icons.

1. **Delete the old file**: `lib/screens/icons/gallery_icons_data.dart`.
2. **Run the web scraper script** from the `tool` folder to generate a fresh file:

* On **Windows CMD / PowerShell / Terminal**:

```bash
dart run tool/web_scraper.dart
```
...
> This will automatically generate the latest `gallery_icons_data.dart` with all Cupertino icons.
...
The Cupertino Icons Gallery was initially developed by [Mugisa Brian Cephas](https://twitter.com/xephas_official), join him on X (*formerly Twitter*) [@xephas_official](https://twitter.com/xephas_official)
